...
kernel_out_dir = $(base_dir)/kernel-outputs
...
# compile verilator
VERILATOR = verilator --cc --exe --trace
# VERILATOR = verilator --cc --exe
VERILATOR_FLAGS = --assert -Wno-STMTDLY -O3 \
...
kernel_bmark_elf ?= $(base_dir)/kernel-bmark/kernel.elf
kernel_bmark_out  = $(patsubst %.elf,%.out,$(kernel_out_dir)/$(notdir $(kernel_bmark_elf)))
$(kernel_bmark_out): $(base_dir)/VTile
	mkdir -p $(kernel_out_dir)
	$^ $(kernel_bmark_elf) $(patsubst %.out,%.vcd,$@)

# $^ $(kernel_bmark_elf) $(patsubst %.out,%.vcd,$@) 2> $@
run-kernel-bmark: $(kernel_bmark_out)
...
clean:
	rm -rf $(gen_dir) $(out_dir) $(dfc_out_dir) $(kernel_out_dir) $(fpga_out_dir) test_run_dir

cleanall: clean
	rm -rf target project/target

clean-myout:
	rm -rf $(dfc_out_dir) $(kernel_out_dir) 
