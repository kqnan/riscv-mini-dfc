...
// read_csr、write_csr、swap_csr 在 encoding.h 中定义

// 处理 which 不是 SYS_exit、SYS_stats 的情况
static long handle_frontend_syscall(long which, long arg0, long arg1, long arg2)
{
  ...
  magic_mem[2] = arg1;
  magic_mem[3] = arg2;
  __sync_synchronize();                  // 用于保证前面操作完成后再执行后续操作，该版本过旧
...
// register指定该变量由处理器寄存器保存，这里指定了a0寄存器
// read safe是指 a0作为读目标寄存器就安全？？？
// 将参数reg代表的CSR寄存器值读出到tmp中
#define read_csr_safe(reg) ({ register long __tmp asm("a0"); \
  asm volatile ("csrr %0, " #reg : "=r"(__tmp)); \
  __tmp; })

// 目前实现中，没有stats寄存器，enable应该是false
// 那么，READ_CTR中 counter_names存储“cycle”等18个字符串
// 而且要减去原本的counters值？？？再保存
// uarch等寄存器没有实现
...
// 写tohost寄存器，但为什么左移1位补1
// while 1不就死循环？？？没错，就是要死循环吧
// 此处写入时，code << 1 | 1 是为了当code为0时，也能够使tohost不为0，从而辨别是否仿真结束。
// 仿真读出时，tohost >> 1 以获取正确的code值。
...
// 内嵌控制
// cause: mcause异常原因 ;   epc: mepc 异常指令地址，可用于处理后返回 ;  regs[32]: sp栈指针 
long handle_trap(long cause, long epc, long regs[32])
{
  int* csr_insn;
  asm ("jal %0, 1f; csrr a0, stats; 1:" : "=r"(csr_insn));         // 从dump来看，jal只是跳过了csrr a0, stats指令？？？
  long sys_ret = 0;
  ...
  else if (cause == CAUSE_DFC_FULL) {
      ;
  }
  else if (cause == CAUSE_ILLEGAL_INSTRUCTION &&
      (*(int*)epc & *csr_insn) == *csr_insn)                    // 非法指令什么都不干？？？
    ;
  else if (cause != CAUSE_USER_ECALL)                          // cause = CAUSE_USER_ECALL + PRV，但user PRV（=0）未实现，只有M（=3）模式，故条件成立
    tohost_exit(1337);                                        // 所以仿真模拟中，必然失败退出！
