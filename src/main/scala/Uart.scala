...
class UartController(frequency: Int, baudRate: Int, implicit val p: Parameters) extends Module {
  ...
  BoringUtils.addSource(io.rxChannel.valid,"uartRxValid")

  //test always send 0xa0
  //  io.txChannel.bits := 0xa0.U
...
//      printf("write state : %x \n",Mux(io.txChannel.ready === true.B, 1.U, 0.U))
...
class Receiver(frequency: Int, baudRate: Int) extends Module {
  val io = IO(new Bundle {
    val rxd = Input(UInt(1.W))
  })
  ...
  when(rx.io.channel.fire()) {
    data := rx.io.channel.bits
    //打印出接收到的字符
    printf("%c", rx.io.channel.bits)
  }
}